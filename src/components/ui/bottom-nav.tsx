...
import { BarChart3, Navigation, FileText, ClipboardPlus, ClipboardCheck, TrendingUp } from "lucide-react";

const allItems = [
  { href: "/", label: "Dashboard", icon: BarChart3, adminOnly: false },
  { href: "/survey/new", label: "Tracking", icon: Navigation, adminOnly: false },
  { href: "/survey", label: "Riwayat", icon: FileText, adminOnly: false },
...
export default function BottomNav({ userRole, pendingCount }: { userRole?: string; pendingCount?: number }) {
  ...
  // Sembunyikan bottom bar saat tracking berlangsung agar layar HP penuh & bebas senggolan
  if (path === "/survey/new") return null;
